...
//                setButtonStates(true, false, false, false);
    ...
    private void deleteCustomer (){
        ...
        if(!makh.isEmpty()){
            int confirm = JOptionPane.showConfirmDialog(this,"Bạn có chắc muốn xóa khách hàng này?","Xác nhậc", JOptionPane.YES_NO_OPTION);
            if(confirm == JOptionPane.YES_OPTION){
                if (customerBus.deleteCustomer(makh)){
                    JOptionPane.showMessageDialog(this,"Xóa khách hàng thành cồn!");
                    clearForm();
                    loadTable();
                }
                else{
    ...
    private boolean validataForm(){
        if (txtmakh.getText().isEmpty() || !txtmakh.getText().matches("KH\\d+")){
            JOptionPane.showMessageDialog(this, "Vui lòng nhập mã khách hàng", "Lỗi", JOptionPane.ERROR_MESSAGE);
            txtmakh.requestFocus();
            return false;
        }
        if(txttenkh.getText().isEmpty() || !txttenkh.getText().matches("[\\p{L}\\s]+")){
