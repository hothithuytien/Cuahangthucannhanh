...
//                setButtonStates(true, false, false, false);
    ...
    // Kiểm tra mã nhân viên: bắt đầu bằng "NV" và các ký tự sau là số
    if (txtmanv.getText().isEmpty() || !txtmanv.getText().matches("NV\\d+")) {
        JOptionPane.showMessageDialog(this, "Mã nhân viên phải bắt đầu bằng NV và theo sau là các chữ số", "Lỗi", JOptionPane.ERROR_MESSAGE);
        txtmanv.requestFocus();
        return false;
    }

    // Kiểm tra họ tên: chỉ cho phép chữ cái và khoảng trắng
    if (txthotennv.getText().isEmpty() || !txthotennv.getText().matches("[\\p{L}\\s]+")) {
    ...
    // Kiểm tra ngày sinh định dạng yyyy-MM-dd với tháng 1-12 và ngày 1-30
    String ngaysinhStr = txtngs.getText().trim();
    if (!ngaysinhStr.matches("\\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|30)")) {
        JOptionPane.showMessageDialog(this, "Ngày sinh không hợp lệ. Định dạng phải là yyyy-MM-dd\nTháng từ 01-12, ngày từ 01-30", "Lỗi", JOptionPane.ERROR_MESSAGE);
        txtngs.requestFocus();
        return false;
    }
    ...
    private void btnsearchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnsearchActionPerformed
        if(tfsearch.getText().matches("NV\\d+")){
         searchNVByID();   
        }
        searchStaff();
        // TODO add your handling code here:
